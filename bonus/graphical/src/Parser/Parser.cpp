...
void Parser::_handleStart(std::size_t size, std::shared_ptr<Map> map)
{
    if (size != 20) {
...
void Parser::_handleTurn(std::size_t x, std::size_t y, std::shared_ptr<Map> map)
{
...
void Parser::_handleBegin(std::shared_ptr<Map> map)
{
    // std::ofstream file("output.log", std::ios_base::app);
    // if (file.is_open())
...
void Parser::_handleBoard(std::shared_ptr<Map> map)
{
    std::string line;
    std::vector<std::vector<Cell>>& playGround = map->getMap();
...
{
    std::cout << "name=\"Graphic\", version=\"1.0\", author=\"FPP\", country=\"FR\"" << std::endl;
}

bool Parser::parseCommand(std::string command, std::shared_ptr<Map> map, GameRules& rules)
{
    (void) rules;
